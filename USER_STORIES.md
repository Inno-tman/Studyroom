...
- As a user, I want a **welcome/hero section** that makes it easy to start a focus session (a "lock-in") in a room.
- As a user, I want **quick links to create/open rooms** so that I can drop into a focus session or any room activity with minimal clicks.
...
> A room is a general-purpose space for any shared activity — focused studying, quiet coworking, a social hangout, a gaming session, a watch party, a book club, or whatever the members agree on. Study is just the default example; the same tools (timer, notes, tasks, calls, chat) work for any room theme.

- As a user, I want to **browse and search public rooms** (by name and subject/theme) so that I can find groups or activities that match my interests.
- As a user, I want to **create a room** (name, subject/theme, description, privacy setting) so that I can invite people to focus, hang out, or do any shared activity together.
- As a user, I want **room themes to be unrestricted** (not only academic subjects) so that I can open a room for coworking, gaming, a watch party, or anything else.
- As a user, I want to **make my room private with a join code** so that only invited people can join.
- As a user, I want to **join a room**, including **with a join code for private rooms**, so that I can participate in the activity.
...
- As a room host/co-host, I want to **edit the room** (name, description, subject/theme) so that the room stays up to date.
...
- As a room member, I want to **send and receive messages in real time** so that I can coordinate with roommates/collaborators.
...
- As a user, I want a **Pomodoro-style focus timer** with configurable focus and break durations so that I can focus in structured intervals.
...
- As a user, I want my completed focus sessions to be **recorded and count toward my focus statistics** so that my effort is tracked.
- As a user, I want to **add notes to a completed session** so that I can remember what I covered.
- As a user, I want focus sessions to be **tracked per room** (one open session per room, resumed if I restart) so that focusing in several rooms doesn't get tangled.
...
- As a user, I want the **AI assistant to use the room notes as context** so that answers are personalized to my room material.

## 8. Room Tasks

- As a room member, I want a **shared to-do list for the room** so that we can manage our shared goals together.
...
- As a user, I want an **AI focus/study assistant** that answers academic questions (subject-aware, room-notes-aware) so that I can learn faster.
...
- As a user, I want to **create and manage flashcard decks** (title, description) so that I can build revision materials.
- As a user, I want to **add, edit, remove, and reorder cards** (front/back) so that my decks are correct.
- As a user, I want a **flip-card view** (tap to flip, next/previous, position counter, shuffle) so that I can drill myself.
...
- As a user, I want a **YouTube focus-music player** (search YouTube or paste a link; queue, shuffle, enqueue) so that I can listen while focusing.
- As a user, I want the room player to **keep the screen awake while playing** so that my device doesn't sleep mid-session.
- As a user, I want the player to **collapse to a mini-pill** so that it doesn't block my work.

## 15. Social Feed

- As a user, I want a **timeline feed** of posts from me and my friends so that I can follow what people are up to.
...
- As a user, I want **room-scoped posts** visible only to room members so that rooms have private feed space.
...
- As a room member, I want to **invite a friend to a room** so that they can join my room.
...
- As a user, I want to see who is **online and in which room** via a floating presence dock so that I can find roommates/collaborators.
- As a user, I want the dock to show a **live count of people focusing now** so that I know the app is active.
...
- As a user, I want an **analytics dashboard** so that I can understand my focus habits.
...
- As a user, I want to see **focus time by room**, **daily trends**, and **hourly distribution** so that I can spot patterns.
- As a user, I want **weekly goal tracking** (set a target, progress vs. actual) so that I can plan my workload.
- As a user, I want to **export my focus data as CSV or JSON** so that I can analyze it elsewhere.
...
- As a user, I want to **earn XP for verified focus minutes** (1 XP per minute) so that my focusing is rewarded.
...
- As a user, I want a **room leaderboard** ranked by verified focus minutes, sessions, and streak so that I can compete with my room.
...
- As a user, I want to **connect my Google Calendar or Microsoft Outlook** so that focus events appear in my agenda.
- As a user, I want **verified focus time automatically added to my connected calendar as a single daily event** (extended for later sessions the same day, with auto-sync toggle) so that my schedule stays accurate without manual work.
...
- As a room host, I want **quiet-room alerts** when my room has had no focus activity in 24 hours (at my local 0/6/12/18) so that I can revive the group.
- As a user, I want **accountability-partner suggestions** (weekly pairing of active, non-friends sharing a room) so that I can focus with someone.
- As a user, I want **schedule reminders** before my preferred focus windows so that I start on time.
